...
A hierarchical Transformer that consumes the *raw* action tokens of each hand
instead of aggregated per-chunk features. Bots are most distinguishable in their
temporal patterns (sizing rhythms, postflop response sequences, preflop priors);
a small Transformer over the (action_type, street, actor, amount-bucket,
pot-flow) tokens captures that signal cheaply and is permutation-invariant over
hands within a chunk.

Architecture
------------
For each chunk (list of hands):
    1. Hand encoder: action tokens are embedded (action_type, street, hero/other
       role, alias actor seat, amount bucket, pot-flow bucket, action position,
       position within street, first-in-street flag, learned projection of
       [amount_bb, pot_after_bb, pot_delta_bb]) and passed through a Transformer
       encoder. The hand embedding is an attention-pool over the action
       sequence (Set Transformer PMA, k=1).
    2. Hand-meta fusion: hand embeddings are augmented with a deepest-street
       embedding and a projection of per-hand continuous context.
    3. Chunk encoder: hand embeddings flow through a Transformer encoder with
       hand-level padding masks (permutation invariant), then a second
       attention-pool yields the chunk embedding.
    4. Head: 2-layer MLP -> one logit per chunk -> sigmoid = P(bot | chunk).

``SequenceModelWrapper`` is a sklearn-style estimator with
``fit(chunks, y, sample_weight=None)`` and ``predict_proba(chunks)`` operating on
RAW chunk payloads. It pickles via joblib (state_dict + config) and runs CPU-only
by default.

Uses the same miner-visible canonical payload produced by
``poker44.validator.payload_view.prepare_hand_for_miner`` (identical fields as
the public benchmark hands), so training and serving encode identically.

Tokenizer/architecture adapted from an MIT-licensed peer implementation;
reimplemented here so training and serving share this module path (required for
joblib round-trip of the pickled wrapper).
"""
...
_AMOUNT_BUCKETS = (
    0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0, 8.0, 12.0, 16.0, 24.0, 36.0, 56.0, 84.0, 126.0,
)
...
# payload_view aliases seats to 1..N where N <= metadata.max_seats (<=10).
_ACTOR_ALIAS_VOCAB_SIZE = 11
# Per-action "first action on its street": 0=pad, 1=continuation, 2=first.
...
HAND_META_DIM = 8
#   0: log1p(hero_starting_stack_bb)
#   1: n_distinct_actors / 10.0
#   2: streets_dealt / 4.0
#   3..6: actions_per_street[preflop/flop/turn/river] / cap
#   7: hero_action_share
...
def _sample_hand_indices(total: int, limit: int) -> List[int]:
    ...
    if limit <= 1:
        return [total // 2]
    last = total - 1
    indices = {int(round(i * last / (limit - 1))) for i in range(limit)}
...
def encode_hand(
    ...
    *,
    ...
    for idx in range(n_actions):
        ...
        actor_seat_int = 0
        if isinstance(actor_seat_raw, (int, float, str)) and str(actor_seat_raw).strip():
            try:
                actor_seat_int = int(actor_seat_raw)
            except (TypeError, ValueError):
                actor_seat_int = 0
        actor_role[idx] = _actor_role(actor_seat_int, hero_seat)
        ...
        if bucket is not None:
            ...
            last_street_token = bucket + 1
        if actor_seat_int > 0:
            distinct_actors.add(actor_seat_int)
            if hero_seat and actor_seat_int == hero_seat:
                hero_action_count += 1
...
def encode_chunk(
    ...
    *,
    ...
    cont = np.zeros((max_hands_per_chunk, max_actions_per_hand, CONT_DIM), dtype=np.float32)
    action_mask = np.zeros((max_hands_per_chunk, max_actions_per_hand), dtype=np.bool_)
...
def _collate(batch: List[Tuple[Dict[str, np.ndarray], float, float]]) -> Dict[str, "torch.Tensor"]:
...
class _AttentionPool(nn.Module):
    ...
    def forward(self, x: "torch.Tensor", key_padding_mask: Optional["torch.Tensor"] = None) -> "torch.Tensor":
        ...
        attn_out, _ = self.attn(
            query=query, key=x, value=x, key_padding_mask=safe_mask, need_weights=False
        )
...
class ChunkSetTransformer(nn.Module):
    ...
    def __init__(self, config: SequenceModelConfig) -> None:
        ...
        self.action_type_emb = nn.Embedding(len(_ACTION_TYPE_VOCAB), d_model, padding_idx=0)
        ...
        self.actor_alias_emb = nn.Embedding(_ACTOR_ALIAS_VOCAB_SIZE, d_model, padding_idx=0)
        ...
        self.action_pos_emb = nn.Embedding(int(config.max_actions_per_hand), d_model)
        self.street_pos_emb = nn.Embedding(int(config.max_actions_per_hand), d_model)
        self.first_in_street_emb = nn.Embedding(_FIRST_IN_STREET_VOCAB_SIZE, d_model, padding_idx=0)
        self.cont_proj = nn.Linear(CONT_DIM, d_model)
        self.input_norm = nn.LayerNorm(d_model)
        self.input_dropout = nn.Dropout(config.dropout)

        self.hand_end_emb = nn.Embedding(len(_HAND_END_VOCAB), d_model, padding_idx=0)
        ...
        self.action_encoder = nn.TransformerEncoder(action_layer, num_layers=config.n_action_layers)
        ...
        self.hand_encoder = nn.TransformerEncoder(hand_layer, num_layers=config.n_hand_layers)
...
class SequenceModelWrapper:
    ...
    ``fit(chunks, y, sample_weight=...)`` with RAW chunk payloads (lists of hand
    dicts). ``predict_proba(chunks)`` returns an ``Nx2`` array. Pickles via
    joblib (state_dict + config), CPU-only by default.
    """
    ...
    def _model_inputs(self, batch: Dict[str, "torch.Tensor"]) -> Dict[str, "torch.Tensor"]:
        return {key: batch[key].to(self.device) for key in _TENSOR_KEYS}
    ...
    def fit(
        ...
        train_loader = DataLoader(
            train_ds, batch_size=self.batch_size, shuffle=True, collate_fn=_collate
        )
        val_loader = DataLoader(
            val_ds, batch_size=self.batch_size, shuffle=False, collate_fn=_collate
        )
    ...
    def _evaluate_loss(self, model: ChunkSetTransformer, loader: "DataLoader", loss_fn: "nn.Module") -> float:
    ...
    def predict_proba(self, chunks: Sequence[Sequence[Dict[str, Any]]]) -> np.ndarray:
        ...
        if saved_schema != int(self.config.schema_version):
            raise RuntimeError(
                ...
                f"{int(self.config.schema_version)}. Retrain the sequence model."
            )
    ...
    def predict_chunk_scores(self, chunks: Sequence[Sequence[Dict[str, Any]]]) -> List[float]:
